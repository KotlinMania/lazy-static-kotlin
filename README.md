...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`lazy_static`](https://crates.io/crates/lazy_static).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `lazy_static` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Lazily-initialized statics (legacy; superseded by `OnceLock`)
...
Original work copyrighted by the upstream `lazy_static` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
