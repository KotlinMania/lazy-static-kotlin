# Claude Code Project Instructions — lazy-static-kotlin

## Project Overview

This is **lazy-static-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`lazy_static`](https://crates.io/crates/lazy_static).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/lazy_static/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/lazy_static>
